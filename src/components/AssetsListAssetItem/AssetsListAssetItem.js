...
const AssetsListAssetItem = (props) => {
  const classes = useStyles();

  const startBuyToken = (item) => {
    props.changeActiveElement(item.addr, item.name, item.balance);
    props.startBuyToken(props.svetTokensAmount, props.svetTokenAddress, props.state_web3);
  };

  const startSellToken = (item) => {
    props.changeActiveElement(item.addr, item.name, item.balance);
    props.startSellToken(props.state_web3);
  };

  if (props.indexList !== undefined) {
    const indexJSXList = props.indexList.map((item) => (
      <AssetItem key={item.addr}
        item={item}
        startBuyToken={() => startBuyToken(item)}
        startSellToken={() => startSellToken(item)}
      />
    ));
    return (
