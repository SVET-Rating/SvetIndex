...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    constructor(address _factory, address _WETH) public {
        factory = _factory;
        WETH = _WETH;
        saltContr =  keccak256(abi.encodePacked(type(UniswapV2Pair).creationCode));
    }
        ...
        returns (uint[] memory amounts)
    {
        ...
        _swap(amounts, path, address(this));
        IWETH(WETH).withdraw(amounts[amounts.length - 1]);
                uint balanceE = address(this).balance;
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = sortTokens(input, output);
            UniswapV2Pair pair = UniswapV2Pair(pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
    ...
    function getAmountsIn(uint amountOut, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return getAmountsIn( amountOut, path);
    }
*/
    ...
    function pairFor(address factoryC, address tokenA, address tokenB) public view override returns (address pair) {
        (address token0, address token1) = sortTokens(tokenA, tokenB);
        pair = address(uint(keccak256(abi.encodePacked(
                hex'ff',
                factory,
                keccak256(abi.encodePacked(token0, token1)),
                saltContr//hex'e18a34eb0e04b04f7a0ac29a6e80748dca96319b42c54d679cb821dca90c6303' // init code hash
            ))));
    }

    // fetches and sorts the reserves for a pair
    function getReserves(address factoryC, address tokenA, address tokenB) public view returns (uint reserveA, uint reserveB) {
        (address token0,) = sortTokens(tokenA, tokenB);
