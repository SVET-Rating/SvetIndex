...
contract Index2SwapEth  {
  //  using SafeMathUniswap for uint;
   ...
   // uint8 discount = 99; //% 
    uint16 buyFee; // in tens promille (needs to div by 10000)
    uint16 sellFee; // in tens promille (needs to div by 10000)
      ...
      //  require (reserve1 >= amountRet[1], "No enought tokenTo in pair");
       ...
       function buyIndexforSvetEth (uint _amount,  //in ether 
        ...
        for (uint8 i = 0; i<index.getActivesLen(); i++) {
            ...
            uint256  sumEth4Act = share * _amount ;   // oracle in ether
        ...
        if (buyFee > 0)  {   
            uint fee = _amount * uint(buyFee) / oraclePrice.getLastPrice(address(svetT)) /10000;
        ...
        (bool successE, bytes memory returnData) = address(msg.sender).call{value: msg.value}(abi.encode(bytes4(keccak256("callMe()"))));
    ...
    function sellIndexforSvet (uint _amount, // in index
                             address _indexT,
                             uint256 _miningDelay,
